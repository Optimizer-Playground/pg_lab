...
- Fixed pg_lab sometimes not being able to enforce Memoize hints. As it turns out, Postgres has a hard-coded restriction
  that the outer side of the memoized join must have a cardinality _estimate_ of 2 or greater. If we use pg_lab to create a
  plan based on hinted cardinalities, we might end up with a plan that could not be hinted again.
- Fixed a memory management edge case that caused the backend to crash if queries with and without hint sets were executed in
